...
public class ListCategoriesTest
{
    ...
    public async Task ListCategoriesOk()
    {
        ...
        var outputRepositorySearch = new SearchOutput<Entities.Category>(
            ...
            items: (IReadOnlyList<Entities.Category>)categoriesExampleList,
    ...
    [Theory(DisplayName = nameof(ListCategoriesWithDefaultParameters))]
    [Trait("Application", "ListCategories - Use Cases")]
    [MemberData(
        nameof(ListCategoriesTestDataGenerator.GetInputsWithDefaultParameters),
        parameters: 18,
        MemberType = typeof(ListCategoriesTestDataGenerator)
    )]
    public async Task ListCategoriesWithDefaultParameters(ListCategoriesInput input)
    {
        var categoriesExampleList = _fixture.GetListOfCategories(15);
        var repositoryMock = _fixture.GetRepositoryMock();
